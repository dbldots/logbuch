...
Go to [Jacek Tomaszewski - ionic-cordova-gulp-seed](https://github.com/jtomaszewski/ionic-cordova-gulp-seed). This project is based on his repository.

# About

Log book to record sailing tracks and happenings. Scores points according to German Brandenburgischer Segler e.V. competition regulations.
...
1. Copy `.envrc.android-sample` or `.envrc.ios-sample` to `.envrc` and configure it.
  ...
  * Also, make sure you have all the keys and certificates needed stored in `keys/android/` and `keys/ios/`:
    ...
    * `keys/android/logbuch.keystore`
    * `keys/ios/logbuch_staging.mobileprovision`
    * `keys/ios/logbuch_production.mobileprovision`
